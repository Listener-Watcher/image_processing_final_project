...
def main(data_flag, output_root, num_epochs, gpu_ids, batch_size, download, model_flag, resize, as_rgb, model_path, run,noise,prob):
    ...
    for epoch in trange(num_epochs):
        if noise!='label':        
            ...
            train_loss = train_labelnoise(model, train_loader, task, criterion, optimizer, device, writer,prob)
...
def train(model, train_loader, task, criterion, optimizer, device, writer):
    ...
    return epoch_loss
def train_labelnoise(model, train_loader, task, criterion, optimizer, device, writer,prob):
    total_loss = []
    global iteration
...
if __name__ == '__main__':
    ...
    parser.add_argument('--noise',
                        default='none',
                        help='to name a noise type:none, jitter,gray,flip',
                        type=str)
    parser.add_argument('--prob',
                        default=0,
                        help='noise probability if given noise',
                        type=float)
    ...
    noise = args.noise
    prob = args.prob
    main(data_flag, output_root, num_epochs, gpu_ids, batch_size, download, model_flag, resize, as_rgb, model_path, run,noise,prob)
